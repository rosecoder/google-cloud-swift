...
final class LockTests: EmulatorTestCase {
    ...
    func testLock() async throws {
        ...
        let task1 = Task.detached { [context] () -> ReturnType in
            return try await Redis.lock(key: lockKey, context: context) { () -> ReturnType in
                let start = Date()
        ...
        let task2 = Task.detached { [context] () -> ReturnType in
            return try await Redis.lock(key: lockKey, context: context) { () -> ReturnType in
                let start = Date()
                return (2, start, Date())
            }
